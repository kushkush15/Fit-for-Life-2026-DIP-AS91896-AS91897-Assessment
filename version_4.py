...
canvas = tk.Canvas(root, bg="lightblue") # Creates a canvas widget which acts as a scrollable container for all GUI elements
...
def workout_log():    
    ...
    workout_var = tk.StringVar(workout_log_frame)  # Creates a Tkinter variable to store the selected dropdown value for the workout type
    workout_var.set(workout_choices[0])  # Sets the default selected value in the dropdown to the first item in workout_choices (Athletics)
    ...
    entry_date = DateEntry(workout_log_frame, date_pattern="yyyy-mm-dd")  # Creates a calendar input widget allowing the user to select a date, stored in YYYY-MM-DD format
    ...
    tk.OptionMenu(workout_log_frame, workout_var, *workout_choices).pack()  # Creates a dropdown menu using all values in workout_choices and displays it in the GUI

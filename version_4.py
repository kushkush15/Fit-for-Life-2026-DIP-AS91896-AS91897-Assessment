...
root.option_add('*Entrybox.Background', 'lightblue')
...
def age_validation():
    ...
    if age=="": #If name is empty, continue loop after error message
        ...
        age_label.config(text=(f"{age} is invalid. Alphabet/letters are not allowed, please enter integers/whole numbers only."))
        return False
...
def graph():
    ...
    with open("version_4.txt", "r") as file:
        for line in file:
            ...
            # ----- DATE FIX (handles both formats) -----
            date_str = data[4].strip()
...
#Exit Button
btn = tk.Button(scrollable_frame, text="Exit",  relief="raised", bg="lightblue", command=root.destroy)
